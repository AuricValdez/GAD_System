...
function displayExtensionProposal(data) {
    ...
    console.log("Received extension proposal data:", data);
    console.log("Received signatories data:", data.signatories);
    
    const sections = data.sections;
    const now = new Date();
    
    // Get signatories from the data
    const signatories = data.signatories || {};
    const position = data.position || 'Extension Coordinator';
    
    // Map position to the appropriate name in signatories
    let preparedByName = '';
    ...
    switch(position) {
        ...
        case 'Extension Coordinator':
            preparedByName = signatories.name4 || signatories.name1 || '';
            break;
    ...
    console.log("Selected position:", position, "Mapped name:", preparedByName);
    
    // Determine if we're in dark mode
    const isDarkMode = document.documentElement.getAttribute('data-bs-theme') === 'dark';
    
    // Set signature box colors based on theme
    const signatureBgColor = isDarkMode ? '#333333' : 'white';
    const signatureTextColor = isDarkMode ? 'white' : 'black';
    ...
    <div class="proposal-container">
                ...
                <div style="margin-left: 20px;">
                    ${Array.isArray(sections.IX_assigned_tasks) 
                        ? sections.IX_assigned_tasks.map(task => {
                            // Get person name
                            const personName = task.name || 'Person';
                            
                            // Start HTML with the name
                            let html = `<div style="margin-bottom: 8px; font-weight: bold;">${personName}:</div>`;
                            
                            // Add bullet list for responsibilities
                            html += '<ul style="list-style-type: disc; margin-top: 0; margin-bottom: 15px;">';
                            
                            // Check if we have responsibilities
                            if (Array.isArray(task.responsibilities) && task.responsibilities.length > 0) {
                                // Each responsibility gets its own list item
                                task.responsibilities.forEach(resp => {
                                    // Skip empty items
                                    if (resp && resp.trim()) {
                                        html += `<li style="margin-bottom: 4px;">${resp.trim()}</li>`;
                                    }
                                });
                            } else {
                                // No responsibilities
                                html += '<li>No responsibilities specified</li>';
                            }
        ...
        <!-- Signatures section with theme-responsive colors -->
        <div style="margin-top: 20px;">
            <div style="display: flex; margin-bottom: 0;">
                <div class="signature-section" style="width: 50%; padding: 10px; border: 0.5px solid black; background-color: ${signatureBgColor}; color: ${signatureTextColor};">
                    <p style="margin: 0; font-weight: bold; text-align: center;">Prepared by:</p>
                    <br><br><br>
                    <p style="margin: 0; text-align: center;"><strong>${preparedByName}</strong></p>
                    <p style="margin: 0; text-align: center;">${position || ''}</p>
                    <p style="margin: 0; text-align: center;">Date Signed: _______________</p>
                </div>
                <div class="signature-section" style="width: 50%; padding: 10px; border-top: 0.5px solid black; border-right: 0.5px solid black; border-bottom: 0.5px solid black; background-color: ${signatureBgColor}; color: ${signatureTextColor};">
                    <p style="margin: 0; font-weight: bold; text-align: center;">Reviewed by:</p>
                    <br><br><br>
                    <p style="margin: 0; text-align: center;"><strong>${signatories.name5 || ''}</strong></p>
                    <p style="margin: 0; text-align: center;">${signatories.head_extension_services || ''}</p>
                    <p style="margin: 0; text-align: center;">Date Signed: _______________</p>
                </div>
            </div>
            
            <div style="display: flex; margin-bottom: 0;">
                <div class="signature-section" style="width: 50%; padding: 10px; border-left: 0.5px solid black; border-right: 0.5px solid black; border-bottom: 0.5px solid black; background-color: ${signatureBgColor}; color: ${signatureTextColor};">
                    <p style="margin: 0; font-weight: bold; text-align: center;">Recommending Approval:</p>
                    <br><br><br>
                    <p style="margin: 0; text-align: center;"><strong>${signatories.name2 || ''}</strong></p>
                    <p style="margin: 0; text-align: center;">${signatories.vice_chancellor_rde || ''}</p>
                    <p style="margin: 0; text-align: center;">Date Signed: _______________</p>
                </div>
                <div class="signature-section" style="width: 50%; padding: 10px; border-right: 0.5px solid black; border-bottom: 0.5px solid black; background-color: ${signatureBgColor}; color: ${signatureTextColor};">
                    <p style="margin: 0; text-align: center;">N/A</p>
                </div>
            </div>
            
            <div class="signature-section" style="padding: 10px; border-left: 0.5px solid black; border-right: 0.5px solid black; border-bottom: 0.5px solid black; text-align: center; background-color: ${signatureBgColor}; color: ${signatureTextColor};">
                <p style="margin: 0; font-weight: bold; text-align: center;">Approved by:</p>
                <br><br><br>
                <p style="margin: 0; text-align: center;"><strong>${signatories.name3 || ''}</strong></p>
                <p style="margin: 0; text-align: center;">${signatories.chancellor || ''}</p>
                <p style="margin: 0; text-align: center;">Date Signed: _______________</p>
            </div>
        </div>
...
// Function to update signature box colors when theme changes
function updateSignatureColors() {
    // Check the current theme
    const isDarkMode = document.documentElement.getAttribute('data-bs-theme') === 'dark';
    
    // Select all signature boxes in the preview
    const signatureBoxes = document.querySelectorAll('.proposal-container .signature-section');
    ...
    signatureBoxes.forEach(box => {
        ...
        // Update all paragraph text colors
...
$(document).ready(function() {
    ...
    if (themeToggleButton) {
        themeToggleButton.addEventListener('click', function() {
            // Give time for the theme to change before updating colors
            setTimeout(updateSignatureColors, 100);
        });
    }
}); 