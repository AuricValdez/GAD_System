...
function getNarrativeData($ppas_form_id) {
    try {
        error_log("=== FETCHING COMPREHENSIVE NARRATIVE DATA ===");
        error_log("Searching for data with ppas_form_id: $ppas_form_id");
        $pdo = getConnection();
        ...
        if ($narrative_data) {
            error_log("Found entry in narrative table with ppas_form_id: $ppas_form_id");
        ...
        error_log("No data found in narrative table, falling back to narrative_entries");
        ...
        if ($narrative_entry_data) {
            error_log("Found matching narrative entry in narrative_entries table ID: " . $narrative_entry_data['id']);
        } else {
            error_log("No matching narrative entry found in narrative_entries table with exact title and year, trying fuzzy search");
            ...
            if ($narrative_entry_data) {
                error_log("Found narrative entry with fuzzy title match in narrative_entries table ID: " . $narrative_entry_data['id']);
            } else {
                // 3. Try by ppas_form_id (if that column exists in narrative_entries table)
                error_log("No fuzzy match found, trying by ppas_form_id");
                ...
                if ($narrative_entry_data) {
                    error_log("Found narrative entry by ppas_form_id in narrative_entries table ID: " . $narrative_entry_data['id']);
                } else {
                    // 4. Try by campus as last resort
                    error_log("No ppas_form_id match found, trying by campus as last resort");
                    ...
                    if ($narrative_entry_data) {
                        error_log("Found narrative entry by campus and year in narrative_entries table ID: " . $narrative_entry_data['id']);
                    }
                }
            }
        }
        ...
        if (!isset($narrative_entry)) {
            error_log("ERROR: Failed to find narrative data in any table");
        ...
        error_log("FOUND ENTRY DATA from combined sources");
        if (isset($narrative_entry['id'])) {
            error_log("Entry ID: " . $narrative_entry['id']);
        }
        error_log("Activity Ratings: " . (isset($narrative_entry['activity_ratings']) && !empty($narrative_entry['activity_ratings']) ? 
            substr($narrative_entry['activity_ratings'], 0, 100) . '...' : 'Not found'));
        error_log("Timeliness Ratings: " . (isset($narrative_entry['timeliness_ratings']) && !empty($narrative_entry['timeliness_ratings']) ? 
            substr($narrative_entry['timeliness_ratings'], 0, 100) . '...' : 'Not found'));
        ...
        if (!empty($narrative_entry['activity_ratings'])) {
            ...
            error_log("WARNING: No activity_ratings found in data");
        }
        ...
        if (!empty($narrative_entry['timeliness_ratings'])) {
            ...
            error_log("WARNING: No timeliness_ratings found in data");
        }
        ...
        echo "// Parse timeliness ratings from database\n";
            ...
            if (!empty($narrative_entry['evaluation'])) {
                ...
                error_log("Raw evaluation data: " . substr($evaluation_str, 0, 200));
                
                // Output JavaScript that safely parses the JSON string
                echo "// Parse evaluation data from database\n";
                echo "console.log('SOURCE: Loading evaluation data from narrative_entries table, record ID: " . $narrative_entry['id'] . "');\n";
                echo "try {\n";
                echo "    const dbEvaluationData = JSON.parse('" . addslashes($evaluation_str) . "');\n";
                echo "    console.log('Evaluation data loaded from database:', dbEvaluationData);\n";
                echo "} catch (error) {\n";
                echo "    console.error('Error parsing evaluation data JSON:', error);\n";
                echo "    const dbEvaluationData = null;\n";
                echo "}\n";
            } else {
                echo "console.log('No evaluation data found in DB');\n";
                echo "const dbEvaluationData = null;\n";
            }
            ...
            if (!empty($narrative_entry['photo_paths'])) {
                ...
                if ($decoded !== null && is_array($decoded)) {
                    ...
                    if (strpos($photo_paths_str, ',') !== false) {
                        ...
                        error_log("Split photo_paths by commas into array");
                    } else {
                        // Single path
                        $images_array = [$photo_paths_str];
                        error_log("Using photo_paths as single string");
                    }
                }
            }
            ...
            if (empty($images_array) && !empty($narrative_entry['photo_path'])) {
                ...
                if (is_string($narrative_entry['photo_path'])) {
                    ...
                    if ($decoded !== null && is_array($decoded)) {
                        $images_array = $decoded;
                        error_log("Successfully decoded photo_path JSON into array");
                    } else {
                        $images_array = [$narrative_entry['photo_path']];
                        error_log("Using photo_path as single string");
                    }
                }
            }
            ...
            if (!empty($images_array)) {
                ...
                error_log("Using " . count($images_array) . " images from " . $image_source);
            } else {
                echo "const dbActivityImages = [];\n";
                error_log("No images found in any source (photo_paths, photo_path, activity_images)");
            }
...
console.log('dbEvaluationData from narrative_entries.evaluation:', typeof dbEvaluationData !== 'undefined' && dbEvaluationData !== null);
...
if (dbEvaluationData && typeof dbEvaluationData === 'object') {
    console.log('Using evaluation data from narrative_entries.evaluation');
    ...
    if (dbEvaluationData.activityRatings && !processedActivityRatings) {
        ...
        console.log('Using activity ratings from evaluation data');
    }
    if (dbEvaluationData.timelinessRatings && !processedTimelinessRatings) {
        ...
        console.log('Using timeliness ratings from evaluation data');
    }
}
...
console.log('Will use narrative_entries.evaluation (activity)?', useEvalActivity);
console.log('Will use narrative_entries.evaluation (timeliness)?', useEvalTimeliness);
...
const finalActivityRatings = useDbActivity ? 
                           ...
                           (useEvalActivity ?
                             (console.log('Using activityRatings from evaluation data'), dbEvaluationData.activityRatings) :
                             (narrativeRatings ? 
...
const finalTimelinessRatings = useDbTimeliness ? 
                             ...
                             (useEvalTimeliness ?
                               (console.log('Using timelinessRatings from evaluation data'), dbEvaluationData.timelinessRatings) :
...
console.log('FINAL DATA SOURCE for activity ratings:', 
            ...
            (useEvalActivity ? 'DATABASE: narrative_entries.evaluation (activityRatings)' :
...
console.log('FINAL DATA SOURCE for timeliness ratings:', 
            ...
            (useEvalTimeliness ? 'DATABASE: narrative_entries.evaluation (timelinessRatings)' :
...
";
        ...
        function formatExtensionAgenda(agenda, filterOnlyChecked = false) {
            ...
            if (agenda === undefined || agenda === null) {
                // Check if we have it in the response object
                if (responseData && responseData.agenda) {
                    agenda = responseData.agenda;
                    console.log('Using agenda from responseData:', agenda);
                } else {
                    // Default value if agenda is missing
