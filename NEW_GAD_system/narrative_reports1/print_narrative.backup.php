...
function getNarrativeData($ppas_form_id) {
    try {
        ...
        // Extract ratings from evaluation field if activity_ratings is empty
        if (empty($narrative_entry['activity_ratings']) && !empty($narrative_entry['evaluation'])) {
            error_log("No activity_ratings found, extracting from evaluation field");
            
            $evaluation = $narrative_entry['evaluation'];
            $eval_data = json_decode($evaluation, true);
            
            if ($eval_data) {
                // Extract ratings from combined evaluation data
                if (isset($eval_data['activity'])) {
                    $narrative_entry['activity_ratings'] = json_encode($eval_data['activity']);
                    error_log("Extracted activity ratings from evaluation.activity: " . $narrative_entry['activity_ratings']);
                } else if (isset($eval_data['ratings'])) {
                    $narrative_entry['activity_ratings'] = json_encode($eval_data['ratings']);
                    error_log("Extracted activity ratings from evaluation.ratings: " . $narrative_entry['activity_ratings']);
                } else if (isset($eval_data['Excellent']) || isset($eval_data['Very Satisfactory'])) {
                    // Direct ratings format
                    $narrative_entry['activity_ratings'] = $evaluation;
                    error_log("Using entire evaluation JSON as activity_ratings: " . $narrative_entry['activity_ratings']);
                }
                
                // Extract timeliness ratings
                if (isset($eval_data['timeliness'])) {
                    $narrative_entry['timeliness_ratings'] = json_encode($eval_data['timeliness']);
                    error_log("Extracted timeliness ratings from evaluation.timeliness: " . $narrative_entry['timeliness_ratings']);
                }
            }
        }
        ...
        // Debug what we found
        if (!empty($narrative_entry['id'])) {
            error_log("Found narrative_entry with ID: " . $narrative_entry['id']);
            
            // Debug activity_ratings
            if (!empty($narrative_entry['activity_ratings'])) {
                $activity_ratings_str = $narrative_entry['activity_ratings'];
                error_log("Raw activity_ratings data: " . substr($activity_ratings_str, 0, 200));
...
<?php endif; ?>
    ...
    <script>
        ...
        function displayNarrativeReport(data) {
            ...
            // Ensure proper structure for ratings data
            if (data.activity_ratings) {
                console.log('Raw activity_ratings:', data.activity_ratings);
                if (typeof data.activity_ratings === 'string') {
                    try {
                        data.activity_ratings = JSON.parse(data.activity_ratings);
                        console.log('Parsed activity_ratings from string:', data.activity_ratings);
                    } catch (e) {
                        console.error('Failed to parse activity_ratings JSON:', e);
            ...
            if (data.timeliness_ratings) {
                console.log('Raw timeliness_ratings:', data.timeliness_ratings);
                if (typeof data.timeliness_ratings === 'string') {
                    try {
                        data.timeliness_ratings = JSON.parse(data.timeliness_ratings);
                        console.log('Parsed timeliness_ratings from string:', data.timeliness_ratings);
                    } catch (e) {
                        console.error('Failed to parse timeliness_ratings JSON:', e);
...
function transformRatingsToProperFormat(ratingsData) {
    ...
    try {
        ...
        for (const [standardKey, variations] of Object.entries(ratingMap)) {
            ...
            // Check flat structure format (e.g., excellent_batstateu)
            for (const variation of variations) {
                const batStateUKey = `${variation}_batstateu`;
                const othersKey1 = `${variation}_others`;
                const othersKey2 = `${variation}_other`;
                
                if (data[batStateUKey] !== undefined) {
                    normalizedData[standardKey].BatStateU = parseInt(data[batStateUKey]) || 0;
                }
                
                if (data[othersKey1] !== undefined) {
                    normalizedData[standardKey].Others = parseInt(data[othersKey1]) || 0;
                } else if (data[othersKey2] !== undefined) {
                    normalizedData[standardKey].Others = parseInt(data[othersKey2]) || 0;
                }
            }
        }
        
        // Log the normalized data
        console.log('Normalized ratings data:', normalizedData);
        
        // Check if we actually have any ratings values
        let hasValues = false;
        for (const category of Object.values(normalizedData)) {
            if (category.BatStateU > 0 || category.Others > 0) {
                hasValues = true;
                break;
            }
        }
        ...
        if (!hasValues) {
            console.warn('No rating values found after normalization, using sample data');
            return getSampleRatingsData();
        }
...
const finalActivityRatings = narrativeRatings || processedActivityRatings || ppasEntriesActivityRatings || activityFallback;
const finalTimelinessRatings = narrativeTimelinessRatings || processedTimelinessRatings || ppasEntriesTimelinessRatings || timelinessFallback;
";
...
return 0;
            ...
            function calculateRatingTotal(ratings, ratingType) {
                try {
                    ...
                    console.log(`Calculating total for rating type: ${ratingType}`, ratings);
                    ...
                    // Check the primary format (which should be the standardized one after transformation)
                    if (ratings[properRatingType]) {
                        ...
                        console.log(`Standard format values - BatStateU: ${batStateU}, Others: ${others}`);
                        return batStateU + others;
                    }

                    // Try lowercase format
                    const lowerRatingType = ratingType.toLowerCase();
                    if (ratings[lowerRatingType]) {
                        let batStateU = 0;
                        let others = 0;
                        
                        // Check all possible participant names for BatStateU
                        if (ratings[lowerRatingType].batstateu !== undefined) {
                            batStateU = parseInt(ratings[lowerRatingType].batstateu) || 0;
                        } else if (ratings[lowerRatingType].BatStateU !== undefined) {
                            batStateU = parseInt(ratings[lowerRatingType].BatStateU) || 0;
                        }
                        
                        // Check all possible participant names for Others
                        if (ratings[lowerRatingType].other !== undefined) {
                            others = parseInt(ratings[lowerRatingType].other) || 0;
                        } else if (ratings[lowerRatingType].others !== undefined) {
                            others = parseInt(ratings[lowerRatingType].others) || 0;
                        } else if (ratings[lowerRatingType].Others !== undefined) {
                            others = parseInt(ratings[lowerRatingType].Others) || 0;
                        }
                        
                        console.log(`Lowercase format values - batstateu: ${batStateU}, others: ${others}`);
                        return batStateU + others;
                    }
                    
                    // Try flat structure
                    const underscoreRatingType = properRatingType.replace(/ /g, '_').toLowerCase();
                    const batStateUKey = `${underscoreRatingType}_batstateu`;
                    const othersKey = `${underscoreRatingType}_other`;
                    const othersKey2 = `${underscoreRatingType}_others`;
                    
                    if (ratings[batStateUKey] !== undefined || ratings[othersKey] !== undefined || ratings[othersKey2] !== undefined) {
                        const batStateU = parseInt(ratings[batStateUKey]) || 0;
                        const others = parseInt(ratings[othersKey] || ratings[othersKey2]) || 0;
                        console.log(`Flat format values - ${batStateUKey}: ${batStateU}, ${othersKey}: ${others}`);
                        return batStateU + others;
                    }

                    // Return 0 if no rating found
                    console.log('No rating format matched, returning 0');
return 0;
                } catch (e) {
