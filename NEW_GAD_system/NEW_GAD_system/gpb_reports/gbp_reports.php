...
<body>
    ...
    <script>
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if (response.status === 'success' && response.data) {
                            ...
                            campusSelect.append('<option value="All Campus">All Campuses</option>');
        ...
        function loadBudgetSummary() {
            ...
            const isAllCampus = selectedCampus === 'All Campus' || selectedCampus === 'All';
            ...
            // For any campus selection including 'All Campus', fetch appropriate budget data
            $.ajax({
                ...
                data: {
                    ...
                    year: selectedYear,
                    all_campuses: isAllCampus ? 1 : 0  // Add flag to indicate all campuses request
                },
                dataType: 'json',
                success: function(response) {
                    if (response.status === 'success' && response.data) {
                        // For All Campus view, this will already be the consolidated totals
            ...
            // AJAX call is now moved up to handle both regular and All Campus cases
        }
        ...
        function displayBudgetSummary(data, year) {
            ...
            // Check if we're displaying consolidated data for All Campus
            const isCentral = <?php echo $isCentral ? 'true' : 'false' ?>;
            const selectedCampus = isCentral ? $('#campus').val() : "<?php echo $userCampus ?>";
            const isAllCampus = selectedCampus === 'All Campus' || selectedCampus === 'All';
            
            // Add 'Consolidated' prefix for All Campus view
            const budgetPrefix = isAllCampus ? 'Consolidated ' : '';
        ...
        async function loadReport() {
            ...
            // We'll get budget data for all cases, including All Campus
            
            // Load budget data for both specific campus and All Campus
            const isAllCampus = selectedCampus === 'All Campus' || selectedCampus === 'All';
                ...
                <div class="mb-1">
                    <div class="row">
                        <div class="col-12" style="line-height: 1.1;">
                            ...
                            <p class="mb-0" style="font-size: 10px;"><strong>${selectedCampus === 'All Campus' ? 'Consolidated Total GAA:' : 'Total GAA:'}</strong> ₱${totalGAA.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}</p>
                            <p class="mb-0" style="font-size: 10px;"><strong>${selectedCampus === 'All Campus' ? 'Consolidated Total GAD Fund (5%):' : 'Total GAD Fund (5%):'}</strong> ₱${totalGADFund.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}</p>
                        </div>
                    </div>
                </div>
                ...
                allCategories.forEach(category => {
                    ...
                    if (categoryData.length === 0) {
                        ...
                        categoryData.forEach(item => {
                            ...
                            // Format relevant_agency to remove brackets
                            let formattedRelevantAgency = '';
                            if (item.relevant_agency) {
                                // Try to parse as JSON if it has brackets
                                if (item.relevant_agency.includes('[') || item.relevant_agency.includes(']')) {
                                    try {
                                        const agencyArray = JSON.parse(item.relevant_agency.replace(/[\[\]"]/g, (match) => {
                                            return match === '[' ? '[' : match === ']' ? ']' : '"';
                                        }));
                                        formattedRelevantAgency = agencyArray.join(', ');
                                    } catch (e) {
                                        // If parsing fails, use as is but remove brackets
                                        formattedRelevantAgency = item.relevant_agency.replace(/[\[\]"]/g, '');
                                    }
                                } else {
                                    formattedRelevantAgency = item.relevant_agency;
                                }
                            }
                            ...
                            html += `
                                <tr>
                                    ...
                                    <td style="font-size: 10px; padding: 2px;">${formattedRelevantAgency || ''}</td>
